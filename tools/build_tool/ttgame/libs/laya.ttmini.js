window.wxMiniGame = function (exports, Laya) {
	...
	MiniFileMgr.fs = tt.getFileSystemManager();
	MiniFileMgr.wxdown = tt.downloadFile;
	...
	class MiniLoader extends Laya.EventDispatcher {
	    ...
	    _loadHttpRequestWhat(url, contentType) {
	        ...
	        if (Laya.Loader.preLoadedMap[url])
	            ...
	            var tempurl = Laya.URL.formatURL(url);
	            if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl) && url.indexOf(MiniAdpter.window.tt.env.USER_DATA_PATH) == -1 && (tempurl.indexOf("http://") != -1 || tempurl.indexOf("https://") != -1) && !MiniAdpter.AutoCacheDownFile) {
	...
	class MiniAdpter {
	    ...
	    static postInfoToContext(url, atlaspngUrl, atlasJson) {
	        ...
	        if (fileNativeUrl) {
	            MiniAdpter.window.tt.postMessage({ url: url, atlasdata: postData, imgNativeUrl: fileNativeUrl, imgReadyUrl: textureUrl, isLoad: "opendatacontext" });
	        }
	        else {
